...
class FaceMasker:
    ...
    def add_mask_one(self, image_path, face_lms, template_name, masked_face_path):
        ...
        # render
        face_mask, new_image = render_cy(np.ascontiguousarray(vertices.T), np.ascontiguousarray(new_colors.T),
                                         np.ascontiguousarray(self.prn.triangles.T.astype(np.int64)), h, w)
    ...
    def get_new_texture(self, ref_texture_src, uv_mask_src, texture):
        ...
        if self.is_aug:
            ...
            if np.random.rand() > 0.5:
                ...
                ref_texture_temp[y_offset:256 - y_offset, x_offset:256 - x_offset, :] = cv2.resize(ref_texture,
                                                                                                   target_size)
