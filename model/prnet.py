...
class Conv2d(nn.Module):
    def __init__(self, input_size, in_channel, out_channel, kernel_size=4, stride=1):
        ...
        self.odd_conv = nn.Sequential(
            nn.ConstantPad2d(
                (self.padding_num // 2, self.padding_num // 2 + 1, self.padding_num // 2, self.padding_num // 2 + 1),
                0),
...
class UpBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride, activation_fn='relu'):
        ...
        self.upConvTranspose = nn.ConvTranspose2d(in_channels, out_channels, kernel_size=4, stride=2, padding=1,
                                                  bias=False)
